        ...
        for house in data_list:
            ...
            try:
              #  info_list.append({'Address':soup.address.string,'CSZ':soup.citystatezip.string,'zpid':soup.zpid.string,'zestimate':'{0:,}'.format(int(soup.zestimate.amount.string))})
                info_list.append({'Address':soup.address.string,'CSZ':soup.citystatezip.string,'zpid':soup.zpid.string,'zestimate':soup.zestimate.amount.string})
            except:
        ...
        for zpid in df_data['zpid']:
            ...
            try:
               comps.append("{0}; EST: {1:,}".format(soup.properties.comp.address.street.string,int(soup.properties.comp.zestimate.amount.string)))
                ...
                comps.append("None Found")
            try:
                details.append({'Year Built':soup.yearbuilt.string,'SQ FT':soup.finishedsqft.string,'Baths':soup.bathrooms.string,'Beds':soup.bedrooms.string,'Tax Assess':'{0:,}'.format(int(soup.taxassessment.string)),'Tax Assess Year':soup.taxassessmentyear.string})
            except:
...
df_data = p.getDeepComps(p.getSearch(data_list))
